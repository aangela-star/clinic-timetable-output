...
function inspectImageResource({ response, expectedUrl, expectedSha256 }) {
  const failClosed = { status: JINAN_CMS_RESULTS.CMS_RESPONSE_CONTRACT_UNVERIFIED };
  ...
  if (typeof expectedSha256 !== 'string' || !/^[a-f0-9]{64}$/.test(expectedSha256)) return failClosed;
  if (createHash('sha256').update(bytes).digest('hex') !== expectedSha256) return failClosed;
  try {
...
async function verifyAmbiguousStateBeforeMutation({ state, transport, logger, id, coordinator }) {
  ...
  if (state?.finalImagePath) {
    ...
    if (response.status === 200
      ...
      if (inspectImageResource({ response: imageResponse, expectedUrl: expectedImageUrl, expectedSha256: state.expectedSha256 }).status !== JINAN_CMS_RESULTS.ALREADY_PUBLISHED) {
        const result = manualCheck(state || {});
        safeLog(logger, {
          attemptId: id,
          stage: 'prior-ambiguous-public-verification',
...
async function publishJinanCmsUnlocked(options, coordinator, logger, id) {
  ...
  try {
    ...
    if (submitResult.status !== JINAN_CMS_RESULTS.SUBMIT_SUCCEEDED) {
      const state = manualCheck({ finalImagePath: record.uploadedImageUrl, orphanUploadRisk: true });
      coordinator.ambiguous = { ...state, publicBaseline, expectedSha256 };
  ...
  coordinator.ambiguous = { ...postSubmitState, publicBaseline, expectedSha256 };
  ...
  try {
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      if (verificationResponse.status === 200
        ...
        if (inspectImageResource({ response: imageResponse, expectedUrl: expectedImageUrl, expectedSha256 }).status !== JINAN_CMS_RESULTS.ALREADY_PUBLISHED) {
          continue;
        }
        coordinator.ambiguous = null;
