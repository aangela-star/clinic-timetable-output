...
async function verifyAmbiguousStateBeforeMutation({ state, transport, logger, id, coordinator }) {
  ...
  if (state?.finalImagePath) {
    const publicResult = inspectPublicCurrent({ response, savedTargetUrl: state.finalImagePath });
    if (response.status === 200
...
async function publishJinanCmsUnlocked(options, coordinator, logger, id) {
  ...
  try {
    ...
    const result = safeStatus(JINAN_CMS_RESULTS.VERIFY_FAILED);
    safeLog(logger, { attemptId: id, stage: 'login-submit', status: result.status, errorCode: result.status });
    return result;
  }
  ...
  let submitRequest;
  try {
    submitRequest = buildSubmitRequest(parseCmsEditorForm(freshEditorResponse.body), retryPlan.uploadUrl);
  } catch (error) {
  ...
  coordinator.ambiguous = { ...postSubmitState };
  ...
  try {
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      const publicResult = inspectPublicCurrent({ response: verificationResponse, savedTargetUrl: retryPlan.uploadUrl });
