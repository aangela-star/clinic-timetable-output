...
const JINAN_CMS_LOGIN_DIAGNOSTIC_STAGES = Object.freeze({
...
// Offline contract only: deliberately not connected to publishJinanCms's legacy
// multi-image replacement transport. Human approval is required before live use.
function planPageSrcReplacement({ editorHtml, originalImageHtml, finalImageUrl, pngDataUrl }) {
  const parsed = parseCmsEditorForm(editorHtml);
  const tokens = collectHtmlSequenceTokens(originalImageHtml);
  if (tokens.length !== 1 || tokens[0].start !== 0 || tokens[0].end !== originalImageHtml.length) {
    throw cmsError(JINAN_CMS_RESULTS.FORM_CHANGED);
  }
  const image = imageTokenInfo(tokens[0], { legacyExistingSource: true });
  if (!image || Object.keys(tokens[0].tag.attrs).some(key => !['src', 'alt', 'width', 'height', 'style'].includes(key))) {
    throw cmsError(JINAN_CMS_RESULTS.FORM_CHANGED);
  }
  const target = validateRootRelativeUploadUrl(finalImageUrl);
  if (!target.endsWith('.png') || target === image.src) throw cmsError(JINAN_CMS_RESULTS.FORM_CHANGED);
  const note = parsed.fields.note;
  const imageResourceUrlEncoded = new URL(image.src, JINAN_CMS_CONFIG.origin).href;
  const matches = collectHtmlSequenceTokens(note).filter(token => {
    if (token.kind !== 'tag' || token.tag.name !== 'img') return false;
    try { return new URL(token.tag.attrs.src || '', JINAN_CMS_CONFIG.origin).href === imageResourceUrlEncoded; }
    catch { return false; } // Unrelated icons/remote images remain byte-for-byte unchanged.
  });
  if (matches.length !== 1 || note.slice(matches[0].start, matches[0].end) !== originalImageHtml
      || matches[0].tag.hiddenByAncestor || isHiddenHtmlElement(matches[0].tag.attrs)) {
    throw cmsError(JINAN_CMS_RESULTS.FORM_CHANGED);
  }
  const alt = String(tokens[0].tag.attrs.alt || '').replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const replacement = `<img alt="${alt}" src="${target}" style="width: auto; max-width: 100%; height: auto;" />`;
  const afterNote = note.slice(0, matches[0].start) + replacement + note.slice(matches[0].end);
  parsePngDataUrl(pngDataUrl);
  const bytes = Buffer.from(pngDataUrl.slice('data:image/png;base64,'.length), 'base64');
  return Object.freeze({
    beforeNote: note,
    afterNote,
    imageUrl: imageResourceUrl(target),
    expectedSha256: createHash('sha256').update(bytes).digest('hex'),
    request: Object.freeze({ method: 'POST', url: parsed.action,
      multipartFields: Object.freeze({ ...parsed.fields, note: afterNote }) }),
  });
}

function planPageSrcRestore(plan, freshEditorHtml) {
  const fresh = parseCmsEditorForm(freshEditorHtml);
  if (fresh.fields.note !== plan.afterNote) throw cmsError(JINAN_CMS_RESULTS.FORM_CHANGED);
  return { method: 'POST', url: fresh.action, multipartFields: { ...fresh.fields, note: plan.beforeNote } };
}

function verifyPageSrcReadback(plan, pageResponse, imageResponse) {
  const failed = { status: JINAN_CMS_RESULTS.MANUAL_CHECK_REQUIRED };
  if (pageResponse?.status !== 200 || hasLocation(pageResponse)
      || !exactFinalUrl(pageResponse, JINAN_CMS_CONFIG.publicUrl)) return failed;
  const body = String(pageResponse.body || '');
  if (body.split(plan.afterNote).length !== 2) return failed;
  try {
    const images = collectHtmlSequenceTokens(body).filter(token => token.kind === 'tag'
      && token.tag.name === 'img' && new URL(token.tag.attrs.src || '', JINAN_CMS_CONFIG.origin).href === plan.imageUrl);
    if (images.length !== 1 || images[0].tag.hiddenByAncestor || isHiddenHtmlElement(images[0].tag.attrs)) return failed;
  } catch { return failed; }
  if (inspectImageResource({ response: imageResponse, expectedUrl: plan.imageUrl }).status !== JINAN_CMS_RESULTS.ALREADY_PUBLISHED) return failed;
  const bytes = Buffer.isBuffer(imageResponse.bytes) ? imageResponse.bytes : imageResponse.body;
  if (createHash('sha256').update(bytes).digest('hex') !== plan.expectedSha256) return failed;
  return { status: 'VERIFIED_READBACK' };
}
...
function validateLoginPostResponse(response) {
  ...
  if (response?.location !== './index.php') {
    throw loginDiagnosticError(
...
function loginOnlyPass() {
  return {
    result: 'PASS',
    reasonCode: 'NONE',
    stage: JINAN_CMS_LOGIN_DIAGNOSTIC_STAGES.LOGIN_CONFIRMED,
  };
}

function loginOnlyFail(reasonCode, stage) {
  const safeReason = reasonCode === JINAN_CMS_RESULTS.AUTH_FAILED
    || reasonCode === JINAN_CMS_RESULTS.FORM_CHANGED
    || reasonCode === JINAN_CMS_RESULTS.VERIFY_FAILED
    || JINAN_CMS_LOGIN_DIAGNOSTIC_REASON_CODE_SET.has(reasonCode)
    ? reasonCode
    : JINAN_CMS_RESULTS.VERIFY_FAILED;
  const safeStage = Object.values(JINAN_CMS_LOGIN_DIAGNOSTIC_STAGES).includes(stage)
    ? stage
    : JINAN_CMS_LOGIN_DIAGNOSTIC_STAGES.LOGIN_CONFIRMED;
  return {
    result: 'FAIL',
...
function loginOnlyReasonFromError(error, fallback) {
  return safeLoginDiagnosticReasonCode(error, fallback);
}

async function loginOnlyJinanCms(options = {}) {
  const env = options.env || process.env;
  const username = env[JINAN_CMS_CONFIG.usernameEnvName];
  const password = env[JINAN_CMS_CONFIG.passwordEnvName];
  if (!username || !password) {
    return loginOnlyFail(JINAN_CMS_RESULTS.VERIFY_FAILED, JINAN_CMS_LOGIN_DIAGNOSTIC_STAGES.CREDENTIALS);
  }
  ...
  try {
    ...
    return loginOnlyFail(JINAN_CMS_RESULTS.VERIFY_FAILED, JINAN_CMS_LOGIN_DIAGNOSTIC_STAGES.LOGIN_PAGE);
  }
  if (loginPageResponse.status !== 200
      || !exactFinalUrl(loginPageResponse, JINAN_CMS_CONFIG.loginUrl)
      || hasRawLocation(loginPageResponse)) {
    return loginOnlyFail(JINAN_CMS_RESULTS.VERIFY_FAILED, JINAN_CMS_LOGIN_DIAGNOSTIC_STAGES.LOGIN_PAGE);
  }
  let loginPageBody;
  try {
    loginPageBody = String(loginPageResponse.body || '');
  } catch (_) {
    return loginOnlyFail(JINAN_CMS_RESULTS.VERIFY_FAILED, JINAN_CMS_LOGIN_DIAGNOSTIC_STAGES.LOGIN_PAGE);
  }
  try {
    parseLoginForm(loginPageBody);
  } catch (error) {
    const reasonCode = error?.code === JINAN_CMS_RESULTS.FORM_CHANGED
      ? JINAN_CMS_RESULTS.FORM_CHANGED
      : JINAN_CMS_RESULTS.VERIFY_FAILED;
    return loginOnlyFail(reasonCode, JINAN_CMS_LOGIN_DIAGNOSTIC_STAGES.LOGIN_PAGE);
  }
  ...
  try {
    ...
    return loginOnlyFail(
      loginOnlyReasonFromError(error, JINAN_CMS_RESULTS.VERIFY_FAILED),
      JINAN_CMS_LOGIN_DIAGNOSTIC_STAGES.LOGIN_POST,
    );
  }
  try {
    if (isRecognizedCredentialRejection(loginPostResponse)) {
      return loginOnlyFail(JINAN_CMS_RESULTS.AUTH_FAILED, JINAN_CMS_LOGIN_DIAGNOSTIC_STAGES.LOGIN_POST);
    }
    validateLoginPostResponse(loginPostResponse);
  } catch (error) {
    return loginOnlyFail(
...
// The page-src path is integrated with the existing transport contract, but
// intentionally accepts only explicit local/mock wiring until the live gate.
async function publishPageSrcMock(options, coordinator) {
  const blocked = () => safeStatus(JINAN_CMS_RESULTS.CMS_RESPONSE_CONTRACT_UNVERIFIED);
  if (options.mode !== 'mock' || !options.transport || typeof options.preserveBackup !== 'function') return blocked();
  const transport = options.transport;
  const pilotStore = options.pilotStore;
  if (pilotStore) {
    try {
      // A different instance cannot resume dispatch, even with the same attempt ID.
      if (await pilotStore.read()) return manualCheck({ orphanUploadRisk: true });
    } catch { return safeStatus(JINAN_CMS_RESULTS.VERIFY_FAILED); }
  }
  const env = options.env || {};
  const anonymousGet = (url, responseType) => sendWithCookies(transport, createCookieJar(), {
    method: 'GET', url, responseType,
    headers: { 'cache-control': 'no-cache, no-store', pragma: 'no-cache' },
  });
  const checkedGet = response => {
    if (response.status !== 200 || hasLocation(response)) throw cmsError(JINAN_CMS_RESULTS.VERIFY_FAILED);
    return response;
  };
  const digest = bytes => createHash('sha256').update(bytes).digest('hex');
  let png;
  try { png = parsePngDataUrl(options.pngDataUrl).png; }
  catch { return safeStatus(JINAN_CMS_RESULTS.VERIFY_FAILED); }
  const verify = async plan => {
    const page = await anonymousGet(JINAN_CMS_CONFIG.publicUrl);
    const image = await anonymousGet(plan.imageUrl, 'bytes');
    return verifyPageSrcReadback(plan, page, image).status === 'VERIFIED_READBACK';
  };
  const success = () => ({ status: JINAN_CMS_RESULTS.PUBLISHED, channels: [{ id: 'jinan-website', ok: true }] });
  if (coordinator.ambiguous) {
    const state = coordinator.ambiguous;
    try {
      if (state.pagePlan && state.pagePlan.expectedSha256 === digest(png) && await verify(state.pagePlan)) {
        coordinator.ambiguous = null;
        return success();
      }
    } catch { /* Never re-dispatch a prior uncertain mutation. */ }
    return manualCheck(state);
  }
  if (!env[JINAN_CMS_CONFIG.usernameEnvName] || !env[JINAN_CMS_CONFIG.passwordEnvName]) return safeStatus(JINAN_CMS_RESULTS.AUTH_FAILED);
  let mutationStarted = false;
  let pagePlan;
  let finalImagePath;
  try {
    const publicBefore = checkedGet(await anonymousGet(JINAN_CMS_CONFIG.publicUrl));
    if (!exactFinalUrl(publicBefore, JINAN_CMS_CONFIG.publicUrl)) throw cmsError(JINAN_CMS_RESULTS.VERIFY_FAILED);
    const jar = createCookieJar();
    const login = checkedGet(await sendWithCookies(transport, jar, { method: 'GET', url: JINAN_CMS_CONFIG.loginUrl }));
    if (!exactFinalUrl(login, JINAN_CMS_CONFIG.loginUrl)) throw cmsError(JINAN_CMS_RESULTS.AUTH_FAILED);
    parseLoginForm(login.body);
    await followLoginSuccessLanding(transport, jar, env[JINAN_CMS_CONFIG.usernameEnvName], env[JINAN_CMS_CONFIG.passwordEnvName]);
    const editorGet = async () => {
      const response = await sendWithCookies(transport, jar, { method: 'GET', url: JINAN_CMS_CONFIG.editorUrl });
      const classification = classifyEditorResponse(response);
      if (classification) throw cmsError(classification);
      return response;
    };
    const firstEditor = await editorGet();
    const prepare = (editorHtml, finalImageUrl) => planPageSrcReplacement({ editorHtml,
      originalImageHtml: options.pageSrc.originalImageHtml, finalImageUrl, pngDataUrl: options.pngDataUrl });
    const baseline = prepare(firstEditor.body, '/upload/page-src-preflight.png');
    if (String(publicBefore.body).split(baseline.beforeNote).length !== 2) throw cmsError(JINAN_CMS_RESULTS.FORM_CHANGED);
    const oldImage = imageTokenInfo(collectHtmlSequenceTokens(options.pageSrc.originalImageHtml)[0], { legacyExistingSource: true });
    const oldUrl = new URL(oldImage.src, JINAN_CMS_CONFIG.origin).href;
    const publicTargets = collectHtmlSequenceTokens(publicBefore.body).filter(token => token.kind === 'tag'
      && token.tag.name === 'img' && new URL(token.tag.attrs.src || '', JINAN_CMS_CONFIG.origin).href === oldUrl);
    if (publicTargets.length !== 1 || publicTargets[0].tag.hiddenByAncestor || isHiddenHtmlElement(publicTargets[0].tag.attrs)
        || publicBefore.body.slice(publicTargets[0].start, publicTargets[0].end) !== options.pageSrc.originalImageHtml) {
      throw cmsError(JINAN_CMS_RESULTS.FORM_CHANGED);
    }
    const oldResponse = checkedGet(await anonymousGet(oldUrl, 'bytes'));
    const oldBytes = Buffer.isBuffer(oldResponse.bytes) ? oldResponse.bytes : oldResponse.body;
    if (!exactFinalUrl(oldResponse, oldUrl) || !Buffer.isBuffer(oldBytes) || !oldBytes.length
        || !['image/jpeg', 'image/png'].includes(contentTypeMime(oldResponse))) throw cmsError(JINAN_CMS_RESULTS.VERIFY_FAILED);
    const backup = Object.freeze({ note: baseline.beforeNote, publicHtml: publicBefore.body,
      imageUrl: oldUrl, bytes: Buffer.from(oldBytes), imageSha256: digest(oldBytes), noteSha256: digest(baseline.beforeNote),
      contentType: contentTypeMime(oldResponse) });
    const receipt = await options.preserveBackup(backup);
    if (receipt?.imageSha256 !== backup.imageSha256 || receipt?.noteSha256 !== backup.noteSha256) throw cmsError(JINAN_CMS_RESULTS.VERIFY_FAILED);
    if (pilotStore) await pilotStore.prepare({ backup, originalImageHtml: options.pageSrc.originalImageHtml,
      originalSrc: oldImage.token.tag.attrs.src, originalStyle: oldImage.token.tag.attrs.style || '', pngSha256: digest(png) });
    const upload = buildUploadRequest({ png, callbackNumber: options.callbackNumber ?? 0 });
    mutationStarted = true;
    // Set the latch before dispatch: upload and submit timeouts must not retry.
    coordinator.ambiguous = { ...manualCheck({ orphanUploadRisk: true }), pageSrcMode: true };
    if (pilotStore) await pilotStore.advance('PREPARED', 'UPLOAD_DISPATCHED');
    const uploaded = parseUploadResponse(await sendWithCookies(transport, jar, upload), upload.url, options.callbackNumber ?? 0);
    if (uploaded.status !== JINAN_CMS_RESULTS.UPLOAD_SUCCEEDED) throw cmsError(JINAN_CMS_RESULTS.UPLOAD_FAILED);
    finalImagePath = uploaded.finalImageUrl;
    if (pilotStore) await pilotStore.advance('UPLOAD_DISPATCHED', 'UPLOADED', finalImagePath);
    pagePlan = prepare(firstEditor.body, finalImagePath);
    const uploadedImage = await anonymousGet(pagePlan.imageUrl, 'bytes');
    if (inspectImageResource({ response: uploadedImage, expectedUrl: pagePlan.imageUrl }).status !== JINAN_CMS_RESULTS.ALREADY_PUBLISHED) throw cmsError(JINAN_CMS_RESULTS.VERIFY_FAILED);
    const uploadedBytes = Buffer.isBuffer(uploadedImage.bytes) ? uploadedImage.bytes : uploadedImage.body;
    if (digest(uploadedBytes) !== pagePlan.expectedSha256) throw cmsError(JINAN_CMS_RESULTS.VERIFY_FAILED);
    const publicFresh = checkedGet(await anonymousGet(JINAN_CMS_CONFIG.publicUrl));
    if (!exactFinalUrl(publicFresh, JINAN_CMS_CONFIG.publicUrl) || publicFresh.body !== publicBefore.body) throw cmsError(JINAN_CMS_RESULTS.FORM_CHANGED);
    const freshEditor = await editorGet();
    if (parseCmsEditorForm(freshEditor.body).fields.note !== baseline.beforeNote) throw cmsError(JINAN_CMS_RESULTS.FORM_CHANGED);
    pagePlan = prepare(freshEditor.body, finalImagePath);
    // Caller preserves the exact rollback plan with its backup, before submit.
    const planReceipt = await options.preserveBackup(Object.freeze({ ...backup, pagePlan }));
    if (planReceipt?.imageSha256 !== backup.imageSha256 || planReceipt?.noteSha256 !== backup.noteSha256) throw cmsError(JINAN_CMS_RESULTS.VERIFY_FAILED);
    coordinator.ambiguous = { ...manualCheck({ finalImagePath, orphanUploadRisk: true }), pageSrcMode: true, pagePlan };
    // Response success is not proof of publishing. Even a timeout gets only
    // anonymous readback; there is never another POST in this attempt.
    if (pilotStore) await pilotStore.advance('UPLOADED', 'SUBMIT_DISPATCHED');
    try { await sendWithCookies(transport, jar, pagePlan.request); } catch { /* read back */ }
    if (await verify(pagePlan)) {
      if (pilotStore) await pilotStore.advance('SUBMIT_DISPATCHED', 'VERIFIED');
      coordinator.ambiguous = null;
      return success();
    }
    return manualCheck(coordinator.ambiguous);
  } catch (error) {
    if (mutationStarted) {
      coordinator.ambiguous = { ...manualCheck({ finalImagePath, orphanUploadRisk: true }), pageSrcMode: true,
        ...(coordinator.ambiguous?.pagePlan ? { pagePlan: coordinator.ambiguous.pagePlan } : {}) };
      return manualCheck(coordinator.ambiguous);
    }
    return safeStatus(error?.code === JINAN_CMS_RESULTS.AUTH_FAILED ? JINAN_CMS_RESULTS.AUTH_FAILED
      : error?.code === JINAN_CMS_RESULTS.FORM_CHANGED ? JINAN_CMS_RESULTS.FORM_CHANGED : JINAN_CMS_RESULTS.VERIFY_FAILED);
  }
}
...
async function publishJinanCmsUnlocked(options, coordinator, logger, id) {
  ...
  if (options.pageSrc || coordinator.ambiguous?.pageSrcMode) return publishPageSrcMock(options, coordinator);
...
module.exports = {
  planPageSrcReplacement,
  planPageSrcRestore,
  verifyPageSrcReadback,
  JINAN_CMS_CONFIG,
  JINAN_CMS_RESULTS,
  buildSubmitRequest,
  buildUploadRequest,
